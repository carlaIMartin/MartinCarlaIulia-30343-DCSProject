...
public class Activation implements Serializable {
	...
	public void Activate() throws CloneNotSupportedException {
		...
		if (Operation == TransitionOperation.Multiply)
			Inmultire();
	...
	private void Inmultire(){
			PetriObject tempIndex = util.GetFromListByName(InputPlaceName, Parent.TempMarking);
			PetriObject simpleIndex = util.GetFromListByName(InputPlaceName, Parent.Parent.ConstantPlaceList);



		if (tempIndex instanceof DataFloat) {
			float tempo =  (float)simpleIndex.GetValue();
			float multipliedValue = tempo * tempo;

			DataFloat rezultat = new DataFloat();
			rezultat.SetName(OutputPlaceName);
			rezultat.SetValue(rezultat);
